module ApplicationHelper
		...
		begin
			...
			commissions.each do |commission|
				...
				if bank.present? && bank.commission_percentage.present?
					...
					# Get bank infos required infos for the compute.
					#bank_commission_percentage = bank.commission_percentage 
					bank_commission_percentage = appropriate_bank_commission_rate(bank.name, commission.credit_id)
				end
	...
	def appropriate_bank_commission_rate(bank_name, commission_id)
		begin
			bank = Bank.find_by(name: bank_name)
			...
			if bank_commission_rate_tracker.present?
				...
				bank.hypoplus_commission_percentage
			end
		rescue Exception => e
			puts "Erreur: #{e.message}"
		end
